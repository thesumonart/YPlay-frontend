...
export function TrendingCard({ video, rank }: TrendingCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group flex gap-4 rounded-xl p-3 -mx-3 hover:bg-[var(--surface-secondary)] transition-colors"
    >
      {/* Rank */}
      <div
        className={cn(
          "flex shrink-0 w-10 items-start justify-center pt-1",
          isTop3 ? "text-[var(--primary)]" : "text-[var(--text-secondary)]"
        )}
      >
        <span
          className={cn(
      ...
      <Link
        href={`/watch/${video.id}`}
        className="relative shrink-0 w-40 sm:w-52 md:w-64 aspect-video rounded-xl overflow-hidden bg-[var(--surface-secondary)]"
      >
        <Image
          src={video.thumbnail}
          alt={video.title}
        ...
        {isTop3 && (
          <div className="absolute top-2 left-2 flex items-center gap-1 rounded-md bg-[var(--primary)] px-1.5 py-0.5">
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-0 py-1">
        ...
        <Link
          ...
          className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors w-fit"
        >
          {video.channel.name}
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-[var(--text-secondary)]">
          ...
          <span className="hidden sm:inline capitalize rounded-md bg-[var(--surface-secondary)] px-2 py-0.5 font-medium">
            {video.category}
          </span>
        </div>
