...
import {
  Play, Shuffle, Share2, Lock, Globe, Link2,
...
export function PlaylistHeader({ playlist, onPlay, onShuffle }: PlaylistHeaderProps) {
  ...
  const { icon: VisIcon, label: visLabel } = VISIBILITY_CONFIG[playlist.visibility];
  ...
  return (
    ...
    >
      {/* Cover image */}
      <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-[var(--surface-secondary)] shadow-lg">
        <Image
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-start justify-between gap-2">
          <h1 className="text-xl font-bold text-[var(--text)] leading-snug">{playlist.title}</h1>
        ...
        >
          ...
          <span className="flex items-center gap-1 text-sm font-medium text-[var(--text-secondary)] group-hover/owner:text-[var(--text)] transition-colors">
            {playlist.owner.name}
            {playlist.owner.verified && <CheckCircle2 className="h-3.5 w-3.5" />}
          </span>
        </Link>
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          <Button onClick={onPlay} size="md" className="gap-2 flex-1 sm:flex-none">
            <Play className="h-4 w-4 fill-white" />
            Play all
          </Button>
          <Button onClick={onShuffle} variant="secondary" size="md" className="gap-2 flex-1 sm:flex-none">
          ...
          <Button
            variant="outline"
            size="md"
            className={cn("gap-2", saved && "text-[var(--primary)] border-[var(--primary)]/40")}
