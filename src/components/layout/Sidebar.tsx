...
function NavItem({ href, icon: Icon, label, collapsed, active }: NavItemProps) {
  const item = (
    ...
    >
      <Icon className={cn("shrink-0", active ? "text-[var(--primary)]" : "h-[18px] w-[18px]")} size={18} />
...
export function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="hidden md:flex flex-col fixed left-0 top-[var(--header-height)] bottom-0 z-40 border-r border-[var(--border)] bg-[var(--surface)] overflow-hidden"
    >
      <nav className="flex flex-col gap-1 p-2 flex-1 overflow-y-auto overflow-x-hidden">
        {navSections.map((section, si) => (
          <div key={si} className={cn("flex flex-col gap-0.5", si > 0 && "mt-2")}>
            <AnimatePresence initial={false}>
              {!sidebarCollapsed && section.label && (
                <motion.p
                  ...
                  className="px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-secondary)]"
                >
                  {section.label}
                </motion.p>
              )}
      ...
      <div className="p-2 border-t border-[var(--border)]">
        <Tooltip content={sidebarCollapsed ? "Expand" : "Collapse"} side="right">
          <button
            ...
            className={cn(
              "flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)]",
              "hover:bg-[var(--surface-secondary)] hover:text-[var(--text)] transition-colors",
              sidebarCollapsed && "justify-center px-2"
            )}
            aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}
          >
            <motion.div animate={{ rotate: sidebarCollapsed ? 0 : 180 }} transition={{ duration: 0.25 }}>
